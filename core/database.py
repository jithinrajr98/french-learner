...
# Add all other database functions from your original code...
...
def get_score_history():
    try:
        with sqlite3.connect(DB_PATH, timeout=DB_TIMEOUT) as conn:
            rows = conn.execute("SELECT checked_on, score FROM translation_scores ORDER BY checked_on").fetchall()
        return pd.DataFrame(rows, columns=["Date", "Score"])
    except sqlite3.Error as e:
